...
DT::datatable(summaryToPrint, options = list(dom = 'tp')) |>
  DT::formatStyle(
...
```{r echo = FALSE, results = "asis"}
DT::datatable(params$checkDataset$checkSummary$nonCriticalCheckSummary, options = list(dom = 'tp'))
...
cat(paste0("# ", datasetName, "  \n\n"))
  ...
  print(htmltools::tagList(DT::datatable(.summaryToPrint,options = list(dom = 'tp'))))
  ...
  cat(paste0("## Non-Critical Checks", "  \n\n"))
      ...
      for(.noncriticalCheckId in names(params$checkDataset$nonCriticalChecks[[datasetName]]$nPctList)){
        ...
        cat("\n")
                ...
                if(!(.currCheck$values$n %in% c(0, NULL))){
                  print(htmltools::tagList(DT::datatable(.currCheck$listing,options = list(dom = 'tp'))))
                }
